...
module.exports = async (client) => {
    (await pGlob(`${process.cwd()}/Events/*/*.js`)).map(async (eventFile) => {
        ...
        if (!event.name) return Logger.warn(`⛔ Evento não carregado: sem nome ↓\nFicheiro -> ${eventFile}`);

        if (!eventList.includes(event.name)) {
            return Logger.typo(`⛔ Evento não carregado: erro de caligrafia ↓\nFicheiro -> ${eventFile}`);
        }
